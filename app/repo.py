...
def remember_attachments(conn: sqlite3.Connection, *, paths: list[str], caption: str, vault_root: Path) -> list[int]:
    ...
    for raw_path in paths:
        src = Path(raw_path)
        if not src.exists():
            logger.warning("附件不存在，跳过: %s", raw_path)
            continue
...
def bootstrap_import_vault(conn: sqlite3.Connection, vault_root: Path) -> int:
    ...
    for path in texts_root.rglob("*.md"):
        ...
        try:
            ...
            logger.debug("读取 vault 文本失败: %s - %s", path, e)
            continue
...
def search_advanced(conn: sqlite3.Connection, conditions: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    query = (conditions.get("query") or "").strip()
    try:
        limit = max(1, min(100, int(conditions.get("limit", 20))))
